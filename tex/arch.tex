...
\verb%rmt_server% sends requests to \verb%rmt_client% via a REST interface for various pieces of information, the details of which are explained further in section \ref{sec:rmt_client}.
...
Docker and its uses are explained in section [section reference to docker].
...
In the case that a \verb%None% value is found, this is translated in the template to a user-friendly output, as can be seen in the following code sample:

\begin{lstlisting}[caption={Source code snippet from host.html template file}]
CPU (%)
...
So in this case, the code displays a Bootstrap label with its colour corresponding to its state (described in section....[reference]), or if it does not exist, displays a grey (label-default) label with ``Unknown'' within it.
...
All database transactions are completed through a wrapper, rather than directly via URLs for example.
This adds security to the application as it means there is no chance of someone performing database actions by accident or intentionally in the case of SQL injection.

It also allows for other databases to be used with minimal change to the source code since the database can be initialised using a PostgreSQL \citep{postgresql} database instead of a MySQL one simply by changing the database constructor. ADD TO FUTURE WORK SECTION TO MAKE THIS CONFIGURABLE.
